...
namespace Crypto
{
	CryptoClient::CryptoClient(std::string name) noexcept : name{name}
	{
		...
		AESEncryptedFile = name + "-AESEncrypted.bin";
		AESDecryptedFile = name + "-AESDecrypted.txt";
		RSAEncryptedFile = name + "-RSAEncrypted.bin";
		RSADecryptedFile = name + "-RSADecrypted.txt";
		AESHashEncryptedFile = name + "-AESHashEncrypted.bin";
		AESHashDecryptedFile = name + "-AESHashDecrypted.bin";
		RSAHashEncryptedFile = name + "-RSAHashEncrypted.bin";
		RSAHashDecryptedFile = name + " -RSAHashDecrypted.bin";
		RSASignature = name + "-RSASignature.bin";
		RSAVerification = name + "-RSAVerification.bin";
		Encrypted_SignedFile = name + "-Encrypted-SignedFile.bin";
		Decrypted_SignedFile = name + "-Decrypted-SignedFile.txt";
		Decrypted_VerifiedFile = name + "-Decrypted-VerifiedFile.txt";
	}

	CryptoClient::~CryptoClient()
	{
		std::remove(Decrypted_SignedFile.c_str());
		std::remove("File-Signed.bin");
	}
	...
	void CryptoClient::RSASignFile(const std::string& inputFile)
	{
		RSACrypto::RSASignFile(inputFile, RSASignature, privateKeyPath);
	}

	void CryptoClient::RSASignData(const std::string& inputData)
	{
		RSACrypto::RSASignData(inputData, RSASignature, privateKeyPath);
	}
	...
	std::pair<std::string, bool> CryptoClient::SignAndEncryptFile(const std::string& inputFile)
	{
		RSACrypto::RSASignFile(inputFile, RSASignature, privateKeyPath);
	...
	std::pair<std::string, bool> CryptoClient::SignAndEncryptData(const std::string& inputData)
	{
		RSACrypto::RSASignData(inputData, RSASignature, privateKeyPath);
	...
	std::pair<std::string, bool> CryptoClient::DecryptAndVerifyData(const std::string& inputData, const std::string& keyFile)
	{
		auto decryptionResult = AES::AESDecryptData(inputData, Decrypted_SignedFile, AESKey);
